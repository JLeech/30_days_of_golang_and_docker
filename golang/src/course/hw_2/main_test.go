...
func callDataSignerCrc32(in chan string, out chan string) {
	out <- DataSignerCrc32(<-in)
}

func callDataSignerMd5(in chan string) string {
	return DataSignerMd5(<-in)
}
...
func SingleHash (in, out chan interface{}) {
	...
	mu := &sync.Mutex{}
	for value := range in{
		parsed_value := strconv.Itoa(value.(int))
		wg.Add(1)
		go func(parsed_value string){
			defer wg.Done()
			//wg := &sync.WaitGroup{}
			inMd5 := make(chan string, 1)
			inMd5CrC := make(chan string, 1)
			inCrc32 := make(chan string, 1)
			right_value := make(chan string, 1)
			left_value := make(chan string, 1)
			
			inMd5 <- parsed_value
			inCrc32 <- parsed_value
			
			mu.Lock()
			md5_value := callDataSignerMd5(inMd5)
			mu.Unlock()
			inMd5CrC <- md5_value
			go callDataSignerCrc32(inMd5CrC, right_value)
			//DataSignerCrc32(DataSignerMd5(parsed_value))
			go callDataSignerCrc32(inCrc32, left_value)
			//DataSignerCrc32(parsed_value)
...
func MultiHash (in, out chan interface{}) {
	...
	for value := range in{
		...
		go func(value interface{}){
			defer wg.Done()
			result_channels := [6]chan string{make(chan string, 1),
								  			  make(chan string, 1),
								  			  make(chan string, 1),
								  			  make(chan string, 1),
								  			  make(chan string, 1),
								  			  make(chan string, 1)}
			for iter := range [6]int{0,1,2,3,4,5}{
				go func(iter int){
					in_chan := make(chan string, 1)
					in_chan <- strconv.Itoa(iter)+value.(string)
					go callDataSignerCrc32(in_chan, result_channels[iter])
				}(iter)
			}
			out <- <-result_channels[0]+<-result_channels[1]+<-result_channels[2]+<-result_channels[3]+<-result_channels[4]+<-result_channels[5]
		}(value)
	}
	wg.Wait()
}
...
func executeJob(wg *sync.WaitGroup, cur_job job, in,out chan interface{}){
	defer wg.Done()
	cur_job(in, out)
	close(out)
}
	...
	hashSignJobs := []job{
		job(func(in, out chan interface{}) {
			for _, fibNum := range inputData {
				fmt.Println("INPUT: " + strconv.Itoa(fibNum))
				out <- fibNum
			}
		}),
		job(SingleHash),
